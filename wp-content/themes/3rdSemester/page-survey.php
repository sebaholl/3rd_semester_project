...
$login_url = add_query_arg('redirect_to', rawurlencode($current_url), home_url('/login-2/'));
$reg_url   = add_query_arg('redirect_to', rawurlencode($current_url), home_url('/register/'));
?>
  ...
  <div class="container survey-form">
    ...
    <?php if ( ! is_user_logged_in() ) : ?>
      <div class="notice error">
        <p><?php echo esc_html( $__('You must be logged in to take the survey.') ); ?></p>
        <p>
          <a class="btn btn--dark" href="<?php echo esc_url($login_url); ?>">
            <?php echo esc_html( $__('Log in') ); ?>
          </a>
          <a class="btn btn--outline" href="<?php echo esc_url($reg_url); ?>">
            <?php echo esc_html( $__('Create an account') ); ?>
          </a>
        </p>
      </div>
    <?php else: ?>
