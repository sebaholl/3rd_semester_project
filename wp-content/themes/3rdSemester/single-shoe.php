      ...
      <article <?php post_class('product__wrap'); ?>>
        ...
        <div class="product__grid">
          <div class="product__media">
            <?php
            if (!empty($cover['url'])) echo '<img src="'.esc_url($cover['url']).'" alt="'.esc_attr($title).'">';
            elseif (has_post_thumbnail()) the_post_thumbnail('large',['alt'=>$title]);
            else echo '<div class="img-placeholder">No image</div>';
            ?>
          </div>
          <div class="product__body">
            <?php if ($price): ?><p class="product__price"><?php echo esc_html( omniora_format_price($price) ); ?></p><?php endif; ?>
        ...
        $meta=['relation'=>'OR'];
        foreach($keys as $k){ $meta[]=['key'=>$k,'value'=>$pid,'compare'=>'=']; $meta[]=['key'=>$k,'value'=>'"'.$pid.'"','compare'=>'LIKE']; }
        $q=new WP_Query([
          'post_type'=>'testimonial','posts_per_page'=>3,'post_status'=>'publish',
          'orderby'=>'date','order'=>'DESC','meta_query'=>$meta,
